  ...
  return (
    <div className="auth-page">
      <div className="auth-card">
        ...
        <div className="auth-divider"><span>{t('auth.login.emailDivider')}</span></div>

        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label><FiMail /> {t('auth.login.email')}</label>
            <input type="email" placeholder={t('auth.login.emailPlaceholder')} value={form.email}
              onChange={(e) => setForm({ ...form, email: e.target.value })} required />
          </div>
          <div className="form-group">
            <label><FiLock /> {t('auth.login.password')}</label>
            <div className="password-input">
              <input type={showPassword ? 'text' : 'password'} placeholder={t('auth.login.passwordPlaceholder')}
        ...
        <div className="auth-footer">
          ...
          <span>{t('auth.login.noAccount')} <Link to="/register">{t('auth.login.signUp')}</Link></span>
        </div>
      </div>
    </div>
  );
